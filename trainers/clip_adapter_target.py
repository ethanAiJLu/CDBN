...
class Cosine_dis(nn.Module):
    def __init__(self):
        super(Cosine_dis, self).__init__()
        # self.text_features = text_features

    def forward(self,x1,x2):
        logit = x1@x2.t()
        sim = torch.max(logit,dim=-1).values
        dis = 1 - sim
        return dis
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            ctx_vectors = embedding[0, 1 : 1 + n_ctx, :]# 16,512
...
class Clip_adapter_target(TrainerXU):
    ...
    def get_text_features(self,clip_model):
        ...
        prompts = prompts.to(self.device)
        with torch.no_grad():
            text_features = self.clip_model.encode_text(prompts).type(self.dtype)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    ...
    def build_model(self):
        ...
        self.text_features = self.get_text_features(self.clip_model).to(self.device)
        # NOTE: only give prompt_learner and meta adpater to the optimizer
    ...
    def forward_backward(self, batch_x, batch_u, batch_support_u):
        ...
        logit_u2 = self.alpha * tip_logits + (1-self.alpha) * logit_u2
        logits_support = self.alpha * tip_logits + (1-self.alpha) * logits_support
